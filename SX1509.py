...
_SX1509_RegInputDisableB = (
    0x00  # Input buffer disable register - I/O[15-8] (Bank B) 0000 0000
)
_SX1509_RegInputDisableA = (
    0x01  # Input buffer disable register - I/O[7-0] (Bank A) 0000 0000
)
_SX1509_RegLongSlewB = (
    0x02  # Output buffer long slew register - I/O[15-8] (Bank B) 0000 0000
)
_SX1509_RegLongSlewA = (
    0x03  # Output buffer long slew register - I/O[7-0] (Bank A) 0000 0000
)
_SX1509_RegLowDriveB = (
    0x04  # Output buffer low drive register - I/O[15-8] (Bank B) 0000 0000
)
_SX1509_RegLowDriveA = (
    0x05  # Output buffer low drive register - I/O[7-0] (Bank A) 0000 0000
)
...
_SX1509_RegInterruptMaskB = (
    0x12  # Interrupt mask register - I/O[15-8] (Bank B) 1111 1111
)
_SX1509_RegInterruptMaskA = (
    0x13  # Interrupt mask register - I/O[7-0] (Bank A) 1111 1111
)
...
_SX1509_RegInterruptSourceB = (
    0x18  # Interrupt source register - I/O[15-8] (Bank B) 0000 0000
)
_SX1509_RegInterruptSourceA = (
    0x19  # Interrupt source register - I/O[7-0] (Bank A) 0000 0000
)
...
_SX1509_RegLEDDriverEnableB = (
    0x20  # LED driver enable register - I/O[15-8] (Bank B) 0000 0000
)
_SX1509_RegLEDDriverEnableA = (
    0x21  # LED driver enable register - I/O[7-0] (Bank A) 0000 0000
)
# Debounce and Keypad Engine
_SX1509_RegDebounceConfig = 0x22  # Debounce configuration register 0000 0000
_SX1509_RegDebounceEnableB = (
    0x23  # Debounce enable register - I/O[15-8] (Bank B) 0000 0000
)
_SX1509_RegDebounceEnableA = (
    0x24  # Debounce enable register - I/O[7-0] (Bank A) 0000 0000
)
...
# Miscellaneous
_SX1509_RegHighInputB = (
    0x69  # High input enable register - I/O[15-8] (Bank B) 0000 0000
)
...
class SX1509:
    def __init__(self, address=0x3E, newSetup=True, sclPin=9, sdaPin=8, CPUSpeed=0):
        self.i2c_device = I2C(0, scl=Pin(sclPin), sda=Pin(sdaPin))
    ...
    def pinMode(self, pin, inOut, HV_Mode=False):
        ...
        if (inOut == IO_Types.PIN_TYPE_OUTPUT) or (
            inOut == IO_Types.PIN_TYPE_ANALOG_OUTPUT
        ):
            modeBit = 0
        else:
            modeBit = 1
    ...
    def setInputMode(self, pin, InputMode):
        ...
        if InputMode == IO_Types.PIN_TYPE_INPUT_PULLUP:
            ...
            tempRegData |= 1 << pin
            self._write_reg_16(_SX1509_RegPullDownB, tempRegData)
        elif (InputMode == IO_Types.PIN_TYPE_INPUT_OPEN_DRAIN) or (
            InputMode == IO_Types.PIN_TYPE_ANALOG_OUTPUT
        ):
    ...
    # - linear/log option removed. It was not used anyway.
    def setupBlink(
        self, pin, tOn, tOff, onIntensity, offIntensity, tRise, tFall
    ):  # , log):
    ...
    def keypad(self, rows, columns, sleepTime, scanTime, debounceTime):
        ...
        print(f'_SX1509_RegDirB {tempWord:>016b}')
        tempWord = self._read_reg_16(_SX1509_RegDirB)
        print(f'_SX1509_RegDirB {tempWord:>016b}')
        ...
        print(f'_SX1509_RegOpenDrainA {tempByte:>08b}')
        ...
        print(f'_SX1509_RegPullUpB {tempByte:>08b}')
        tempByte = self._read_reg_8(_SX1509_RegPullUpB)
        print(f'_SX1509_RegPullUpB after {tempByte:>08b}')
        ...
        if sleepTime != 0:
            for i in range(7, 0, -1):
                if sleepTime & (
                    1 << (i + 6)
                ):  # TODO: This might not work, C code had a typecast here...
                    sleepTimeBits = i
                    break
    ...
    def debounceTime(self, time):
        ...
        configValue = 0
        errorValue = (
            10000  # Set this to a number that will always be above the calculated error
        )
    ...
    def checkInterrupt(self, pin):
        if self.interruptSource(False) & (1 << pin):
            return True
        return False
    ...
    def SetLEDClkDivider(self, oscDivider):
        ...
        if oscDivider == 0:
            ...
            self._clkX = self._CPU_CLK / (
                1 << (oscDivider - 1)
            )  # Update private clock variable
    ...
    def _write_reg_8(self, reg, val):
        self.i2c_device.writeto(self.address,bytes([reg, val]))
    
    def _write_reg_16(self, reg, val):
        self.i2c_device.writeto(self.address,bytes([reg, ((val>>8)&(0xFF)), (val&0xFF)]))

    def _read_reg_8(self, reg):
        self.i2c_device.writeto(self.address,bytes([reg]))
        result = bytearray(1)
        result = self.i2c_device.readfrom(self.address,1)
        return result[0]
    
    def _read_reg_16(self, reg):
        self.i2c_device.writeto(self.address,bytes([reg]))
        result = bytearray(2)
        result = self.i2c_device.readfrom(self.address,2)
